...
import { fetchPage } from "../../apis/firebase";
...
const langName1 = "eng"; // Поле по умолчанию для сортировки (заменяем "name" на fieldName)
const langName2 = "ukr";
...
const PaginatedList = () => {
    ...
    const [sortField, setSortField] = useState(langName1); // Поле для сортировки
    const [searchQuery, setSearchQuery] = useState(""); // Поисковый запрос

    const [currentLang, setCurrentLang] = useState(langName1);
    ...
    const fetchFirstPage = async () => {
        ...
        const querySnapshot = await fetchPage(fireDB, "lang",sortField,pagesLimit,searchQuery,null,'first',level);
    ...
    const fetchNextPage = async () => {
        ...
        const querySnapshot = await fetchPage(fireDB, "lang",sortField,pagesLimit,searchQuery,lastVisible,'next',level);
    ...
    const fetchPrevPage = async () => {
        if (currentPage > 0) {
            ...
            const querySnapshot = await fetchPage(fireDB, "lang",sortField,pagesLimit,searchQuery,visibleDocs[currentPage - 1],'prev',level);
    ...
    const fetchLastRecord = async () => {
        ...
        //
        // const querySnapshot = await getDocs(q);
        const querySnapshot = await fetchPage(fireDB, "lang",sortField,pagesLimit,searchQuery,null,'last',level);
    ...
    //****
        ...
        const randomAllDocs = query(collectionRef, where('level', "==", level));
        ...
        const randomQuery = query(collectionRef, where('level', "==", level), limit(randomIndex + 1));
    ...
    const changeCart = async () => {
        if (currentLang === langName1) {
            setCurrentLang(langName2)
        } else {
            setCurrentLang(langName1)
        }
    }
    ...
    return (
        <div>
            ...
            <div className="d-flex justify-content-start mb-3 gap-5">
            ...
            <div className="d-flex justify-content-between mt-4">
                <div className="d-flex justify-content-start gap-2">
                    <button className="btn btn-warning" onClick={getRandomDocument}>Random</button>
                    <button className="btn btn-secondary" onClick={fetchFirstRecord} disabled={isFirstDisabled}>First</button>
                    <button className="btn btn-primary" onClick={fetchPrevPage} disabled={isPrevDisabled}>Previous</button>
                    <button className="btn btn-primary" onClick={fetchNextPage} disabled={isNextDisabled}>&nbsp;&nbsp;Next&nbsp;&nbsp;</button>
                    {/*<button className="btn btn-primary" onClick={fetchLastRecord} disabled={isLastDisabled}>Последняя запись</button>*/}
                </div>
            </div>
        </div>
    );
